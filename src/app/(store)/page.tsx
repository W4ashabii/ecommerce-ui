...
import { productsApi, settingsApi, categoriesApi, Product } from '@/lib/api';
...
export default function HomePage() {
  ...
  // Get the selected category for featured collection
  const featuredCategory = settings?.featuredCollection?.collectionId
    ? categories?.find(c => c._id === settings.featuredCollection?.collectionId)
    : null;
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="py-20 bg-card text-foreground overflow-hidden border-y border-border">
        <div className="container mx-auto px-4">
          {settings?.featuredCollection?.isActive !== false && (
            <div className="grid lg:grid-cols-2 gap-12 items-center">
              ...
              >
                <span className="text-brand-pink text-sm font-medium tracking-wider uppercase">
                  {settings?.featuredCollection?.label || 'Featured Collection'}
                </span>
                <h2 className="text-4xl md:text-5xl font-display font-bold mt-4 mb-6">
                  {settings?.featuredCollection?.title || 'Summer'}
                  {settings?.featuredCollection?.titleHighlight && (
                    <>
                      <br />
                      <span className="text-brand-pink">{settings.featuredCollection.titleHighlight}</span>
                    </>
                  )}
                </h2>
                <p className="text-muted-foreground mb-8 max-w-md">
                  {settings?.featuredCollection?.description || 'Embrace the season with our curated selection of lightweight fabrics and vibrant designs perfect for warm days.'}
                </p>
                <Button size="lg" asChild>
                  <Link href={
                    settings?.featuredCollection?.collectionId
                      ? `/shop?category=${settings.featuredCollection.collectionId}`
                      : '/collections'
                  }>
                    {settings?.featuredCollection?.buttonText || 'Explore Collection'}
                    <ArrowRight className="ml-2 h-5 w-5" />
                  </Link>
                </Button>
              </motion.div>
